...
def color_guess(correct_color, attempts=3):
    ...
    while attempts > 0: # Loop for limited attempts
        ...
        if is_correct_guess(guess, correct_color): # Check if guess is correct
            print("Hero smiles: 'Yes! You do remember!'")
            return True
        else:
...
def intro_scene():
    ...
    typingPrint("...")
    time.sleep(1)
    typingPrint("\nWelcome to WHITE SPACE🔪", color=PINK)
    time.sleep(1)
...
def forest_scene():
    ...
    while True:
        ...
        if direction == "right":
            confirm = input("Hero: 'Are you sure?......' (yes/no): ").strip().lower()
